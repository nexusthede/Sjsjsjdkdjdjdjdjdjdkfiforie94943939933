...
const embedMsg = (desc) => new EmbedBuilder().setColor("#00008B").setDescription(desc);

// Voice State Update - Join and Leave VC Logic
const voiceStateUpdate = async (client, oldState, newState, config) => {
  if (newState.guild.id !== config.GUILD_ID) return;
  const userId = newState.id;

  // Handle joining a voice channel (VC creation if needed)
  if (!oldState.channelId && newState.channelId) {
    vcData.vcJoinTimestamps[userId] = Date.now();

    if (newState.channelId === config.JOIN_TO_CREATE_ID) {
      // Prevent creating a duplicate VC
  ...
  // Handle leaving a voice channel (Delete if no members left)
  if (oldState.channelId && !newState.channelId) {
...
const messageCreate = async (client, message, config) => {
  ...
  // Command for managing voice channels
  if (cmd === "vc") {
  ...
  // List all VCs
  if (cmd === "vc list") {
    const vclist = Object.entries(vcData.vcOwners)
      .map(
        ([id, owner]) =>
          `VC: ${message.guild.channels.cache.get(id)?.name || "Deleted"} | Owner: <@${owner}>`
      )
      .join("\n") || "No active VCs";
